...
function Slarm_send_ajax(el, action, cdata) {
    var data = {};
...
function Slarm_woo_stamped_bulk_reviews(resp, el, status) {
    console.log(status);
...
function Slarm_woo_stamped_clear_reviews_cache(resp, el, status) {
    el.find(".spinner").css({ "visibility": "hidden" });
    if (status.status >= 200 && status.status < 400) {
        if (typeof resp != "undefined" && resp != "") {
            resp = jQuery.parseJSON(resp);
...
function Slarm_woo_stamped_bulk_export_review(resp, el, status) {
    console.log(status);
    el.find(".spinner").css({"visibility": "hidden"});
    if (status.status >= 200 && status.status < 400) {
        if (typeof resp != "undefined" && resp != "") {
...
Woo_stamped.addAction("send_ajax_after_success", function (resp, status, jqXHR, action, el) {
    if (action == "Woo_stamped_bulk_reviews") {
        Slarm_woo_stamped_bulk_reviews(resp, el, jqXHR);
    }

    if (action == "Woo_stamped_clear_reviews_cache") {
        Slarm_woo_stamped_clear_reviews_cache(resp, el, jqXHR);
    }

    if (action == "Woo_stamped_bulk_export_review") {
        Slarm_woo_stamped_bulk_export_review(resp, el, jqXHR);
    }
...
Woo_stamped.addAction("send_ajax_after_error", function (jqXHR, status, errorThrown, action, el) {
    if (action == "Woo_stamped_bulk_reviews") {
        Slarm_woo_stamped_bulk_reviews("", el, jqXHR);
    }

    if (action == "Woo_stamped_clear_reviews_cache") {
        Slarm_woo_stamped_clear_reviews_cache("", el, jqXHR);
    }

    if (action == "Woo_stamped_bulk_export_review") {
        Slarm_woo_stamped_bulk_export_review("", el, jqXHR);
    }
});
...
jQuery(document).ready(function () {
    jQuery("body").on("click", "#Woo_stamped_bulk_review_button", function (e) {
        ...
        Slarm_send_ajax(elparent, "Woo_stamped_bulk_reviews");
    });
    ...
    jQuery("body").on("click", "#Woo_stamped_clear_reviews_cache", function (e) {
        ...
        elparent.find(".response").html("");
        jQuery(this).attr("disabled", "disabled");
        Slarm_send_ajax(elparent, "Woo_stamped_clear_reviews_cache");
    });
    ...
    jQuery("body").on("click", "#Woo_stamped_bulk_export_review_button", function (e, v) {
        ...
        elparent.find(".response").html("");
        jQuery(this).attr("disabled", "disabled");
        Slarm_send_ajax(elparent, "Woo_stamped_bulk_export_review");
    });
