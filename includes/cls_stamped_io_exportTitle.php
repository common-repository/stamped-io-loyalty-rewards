...
class Woo_stamped_io {
    ...
    public function __construct() {
        ...
        $this->pl_url = SLARM_WOO_STAMPED_URL;     
        $this->Slarm_woo_stamped_global_init();
        if (is_admin()) {
            $this->Slarm_woo_stamped_admin_init();
        } else {
            $this->Slarm_woo_stamped_public_init();
        }
    }
     ...
     */
    public function Slarm_woo_stamped_global_init() {
        $this->Slarm_woo_stamped_activate_order_status();
        add_shortcode("Woo_stamped_io", array($this, "Slarm_woo_stamped_io_shortcode"));
    }

    /**
     * Admin Setting for admin
     */
    public function Slarm_woo_stamped_admin_init() {
        add_action('admin_enqueue_scripts', array($this, 'Slarm_woo_stamped_admin_js'));
        $this->Slarm_woo_stamped_admin_ajax();
    }

    /**
     * Public Setting for Public
     */
    public function Slarm_woo_stamped_public_init() {
        $this->Slarm_woo_stamped_prepare_scripts();
    }

    /**
     * All Wp Ajax Hooks
     */
    public function Slarm_woo_stamped_admin_ajax() {
        add_action("wp_ajax_Woo_stamped_bulk_reviews", array($this, "Slarm_woo_stamped_bulk_reviews_ajax"));
        add_action("wp_ajax_nopriv_Woo_stamped_bulk_reviews", array($this, "Slarm_woo_stamped_bulk_reviews_ajax"));
        add_action("wp_ajax_Woo_stamped_bulk_export_review", array($this, "Slarm_woo_stamped_bulk_export_review"));
        add_action("wp_ajax_nopriv_Woo_stamped_bulk_export_review", array($this, "Slarm_woo_stamped_bulk_export_review"));
        add_action("wp_ajax_Woo_stamped_clear_reviews_cache", array($this, "Slarm_woo_stamped_clear_reviews_cache"));
        add_action("wp_ajax_nopriv_Woo_stamped_clear_reviews_cache", array($this, "Slarm_woo_stamped_clear_reviews_cache"));
    }

    public function Slarm_woo_stamped_admin_js() {
        ...
        $this->Slarm_woo_stamped_admin_localize_scripts();
    }

    public function Slarm_woo_stamped_admin_localize_scripts() {
        $vars = array();
        $vars["ajax_url"] = admin_url("admin-ajax.php");
        wp_localize_script('woo-stamped-io-admin-js', 'Woo_stamped_admin', $vars);
    }

    public function Slarm_woo_stamped_prepare_scripts() {
        add_action('wp_enqueue_scripts', array($this, 'Slarm_woo_stamped_public_js_css'));
    }

    public function Slarm_woo_stamped_public_js_css() {
        ...
        $this->Slarm_woo_stamped_localize_vars();
    }

    public function Slarm_woo_stamped_localize_vars() {
        $vars = array();
        $vars["pub_key"] = Woo_stamped_api::Slarm_get_public_keys();
        $vars["url"] = Woo_stamped_api::Slarm_get_site_url();
        wp_localize_script('woo-stamped-io-public-custom', 'Woo_stamped', $vars);
    }

    public function Slarm_woo_stamped_io_shortcode($atts, $content = "") {
        global $post;
        $atts = shortcode_atts(array("type" => 'badge', "product_id" => 0), $atts, 'Slarm_woo_stamped_io_shortcode');
        extract($atts);
        global $product;
        ...
        if ($type == 'badge') {
            ob_start();
            Woo_stamped_public::Slarm_woo_stamped_review_badge();
            return ob_get_clean();
        }
        if ($type == 'widget') {
            ob_start();
            Woo_stamped_public::Slarm_woo_stamped_review_box();
            return ob_get_clean();
        }
        if ($type == 'rich-snippet') {
            $data = $this->Slarm_woo_stamped_fetched_aggregate_rating($product->id);
            return Woo_stamped_public::Slarm_woo_stamped_aggregate_rating($data);
        }
    }

    public function Slarm_woo_stamped_bulk_reviews_ajax() {
        $response = $this->Slarm_woo_stamped_bulk_review();
        $return = array("status" => "success", "text" => "<b>Order history exported to Stamped.io</b>");
        echo json_encode($return);
        exit();
    }

    public function Slarm_woo_stamped_activate_order_status() {
        $statuses = Woo_stamped_api::Slarm_get_activated_status();
        if (is_array($statuses) && count($statuses) > 0) {
            foreach ($statuses as $status) {
                add_action("woocommerce_order_status_{$status}", array($this, 'Slarm_woo_stamped_submit_review'), 10);
            }
        }
    }

    public static function Slarm_woo_stamped_order_json($ID) {
        $orderData = array();
        ...
        if ($order) {
            ...
            if (is_array($line_items) && count($line_items) > 0) {
                foreach ($line_items as $key => $v) {
                    ...
                    if ($productId != "") {
                        ...
                        $url = get_the_post_thumbnail_url($products->id);
                        $items[] = array("productId" => $productId,
                            ...
                            "productTags" => strip_tags($products->get_tags()),
            ...
            $orderData = array(
                ...
                'orderId' => $ID,
                'orderCurrencyISO' => $order->get_order_currency(),
    ...
    public function Slarm_woo_stamped_submit_review($ID) {
        $orderData = self::Slarm_woo_stamped_order_json($ID);
        //Woo_stamped_api::Slarm_Woo_stamped_logging("single call before start  array " . print_r($orderData, true));
        //Woo_stamped_api::Slarm_Woo_stamped_logging("single call before start json  " . json_encode($orderData, true));
        $response = Woo_stamped_api::Slarm_send_request("/survey/reviews", $orderData);
        //Woo_stamped_api::Slarm_Woo_stamped_logging("single call after success " . print_r($response, true));
        if (is_object($response)) {
            update_post_meta($ID, "send_to_stamped_for_review", 1);
        }
    }

    public function Slarm_woo_stamped_fetched_aggregate_rating($product_id) {
        ...
        if ($agrr_review == null || $agrr_review == "" || $ttl < time()) {
            $outcome = (array) Woo_stamped_api::Slarm_send_request("/richSnippet?productId={$product_id}", array(), "GET");
    ...
    public function Slarm_woo_stamped_bulk_review() {
        $posted_data = Slarm_woo_stamped_cstm_sanitize_array($_POST);
        extract($posted_data);
        $per_page = 500;
        $response = array();
        $status = Woo_stamped_api::Slarm_stamped_order_status();
        $args = array(
        ...
        //Woo_stamped_api::Slarm_Woo_stamped_logging("found-post {$orders->found_posts}");
        if ($orders->have_posts()) {
            while ($orders->have_posts()) {
                $orders->the_post();
                global $post;
                $bulkorderData[] = self::Slarm_woo_stamped_order_json($post->ID);
            }
        }
        $response = "";
        if (is_array($bulkorderData) && count($bulkorderData) > 0) {
            $response = Woo_stamped_api::Slarm_send_request("/survey/reviews/bulk", $bulkorderData);

            //Woo_stamped_api::Slarm_Woo_stamped_logging("Bulk call response " . print_r($response, true));


            if (is_object($response) && count($response) > 0) {
                update_options("update_stamped_bulk_order", date("Y-m-d"));
                //Woo_stamped_api::Slarm_Woo_stamped_logging("Bulk call success ");
...
//        var_dump($paged);
    ...
    public function Slarm_woo_stamped_clear_reviews_cache() {
        global $wpdb;
    ...
    public function Slarm_woo_stamped_bulk_export_review() {
        $posted_data = Slarm_woo_stamped_cstm_sanitize_array($_POST);
        extract($posted_data);
        global $wpdb;
        ...
        if (is_array($result) && count($result) > 0) {
            foreach ($result as $key => $val) {
                ...
                //Custom user id filter
                if ( !empty($comment_author) ) {
                	$tmp_comment = get_comment($comment_ID);
                	$user_id = $tmp_comment->user_id;
			if (!empty($user_id)){
				$user = get_userdata($user_id);
				$comment_author = ucwords(strtolower($user->first_name)).' '.ucfirst(strtolower(substr($user->last_name,0,1))); // this is the actual line you want to change
			} else {
				$author_name = $comment_author;
				$parts = explode(" ", $author_name);
				$comment_author = ucwords(strtolower($parts[0]).' '.(isset($parts[1]) ? ucfirst($parts[1][0]) : ''));
			}
		} else {
			$comment_author = __('Customer');
		}
		
		//Custom title
		if( ! $comment_title = get_comment_meta($comment_ID, "title", true) ) {
			$comment_pieces = explode(" ", $comment_content);
			$first_five = implode(" ", array_splice($comment_pieces, 0, 5));
			$comment_title = $first_five;
		}


                $comments[$comment_ID] = array($comment_post_ID, $comment_author, $comment_author_email, get_comment_meta($comment_ID, "rating", true), $comment_title, $comment_content, $comment_date, ($comment_parent == "0" ? '' : 1 ), '', $product_image, $product_url, $is_published);
                if ($comment_parent != 0) {
